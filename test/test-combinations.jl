...
@testset "p-Value combinations" begin
    ...
    ref1 = Dict(FisherCombination   => 0.01558752, # `metap::sumlog(p)`
        ...
        SimesCombination    => 0.04)

    p2 = [0.0001, 0.001, 0.01, 0.05, 0.1, 0.2, 0.4, 0.75]
    ref2 = Dict(FisherCombination   => 1.285885e-06, # `metap::sumlog(p)`
        ...
        SimesCombination    => 0.0008)

    p3 = copy(p1)
    w3 = [0.1, 0.2, 0.3, 0.4] .* 2
    ref3 = Dict(StoufferCombination => 0.1916892)

    p_adjust_combinations = Dict(Sidak             => TippettCombination,
